...
\begin{tabular}{| c | c |}
	...
	\hline
	$s(1)$ & 
	\ON<2->{\B{The position of $\mathcal{O}$ after 1s}}\\
	\hline
	$s(0)$ & 
	\ON<3->{\B{The initial position of $\mathcal{O}$}}\\
	\hline
	\ON<4->{\B{$s(6)$}} & 
	The position of $\mathcal{O}$ after 6s\\
	\hline
	$s(7)-s(4)$ & 
	\ON<5->{\B{$\mathcal{O}$'s change position (distance traveled) between 4s and 7s}}\\
	\hline
	\ON<6->{\B{$s(3)-s(0)$}} & 
...
$\D{\bar{v} = 
\ON<7->{\B{\frac{
\only<8->{\OS{\B{ change \, in \, position }}}{
\only<8->{\OBT}{ s(b)-s(a) }}}{ 
\only<9->{\US{\B{ change \, in \, time }}}{
\only<9->{\UBT}{\, b-a \,}}}}}}$	
...
\begin{tabular}{p{0.25cm}p{13.6cm}}
	...
	\\
	& \ON<2->{\B{$\D{s(1) = -16(1)^2+96(1) = -16+96 = 80 \, ft}$}} \\
	& \\
	(b) & What was average velocity of the rock between 1 s and 3 s? \\
	\\
	& \ON<3->{\B{$\D{\bar{v}_{1 \to 3} = \frac{s(3)-s(1)}{3-1} = \frac{[-16(3)^2+96(3)]-[-16(1)^2+96(1)]}{2} = \frac{144-80}{2} = \frac{64}{2} = 32 \, ft/s}$}} \\
	& \\
	(c) & What was average velocity of the rock between 1 s and 2 s? \\
	\\
	& \ON<4->{\B{$\D{\bar{v}_{1 \to 2} = \frac{s(2)-s(1)}{2-1} = \frac{[-16(2)^2+96(2)]-[-16(1)^2+96(1)]}{1} = \frac{128-80}{1} = 48 \, ft/s}$}} \\
	& \\
	(d) & What was average velocity of the rock between 1 s and $k$ s? (assume $k>1$)\\
	\\
	& \ON<5->{\B{$\D{\bar{v}_{1 \to k} = \frac{s(k)-s(1)}{k-1} = \frac{-16k^2+96k - 80}{k-1} \, ft/s}$}} \\
\end{tabular}}
...
\begin{tabular}{cp{0.2cm}c}
	\multirow{2}{*}{
	\only<1-2>{\includegraphics[scale = 0.27]{Figures/01_01_Fig_04.png}}
	\only<3->{\includegraphics[scale = 0.27]{Figures/01_01_Fig_05.png}}} & & 
...
\begin{tabular}{p{0.25cm}p{14.6cm}}
	...
	&\\
	& 
	\ON<2->{\B{$\D m_S = \frac{1-(-2)}{2-6} = -\frac{3}{4}$}}\\
	&\\
	(b) & Under what condition could this slope above be considered an average velocity? \\
	&\\
	& 
	\ON<3->{\B{If $f(x)$ was a position function of $x$ (time)}}\\
\end{tabular}}
...
\begin{tabular}{p{0.25cm}p{14.6cm}}
	...
	\ON<4->{\B{ slope, $m$, and a point, $(x_1,y_1)$, on the line}} which we plug into: 
	\ON<5->{\B{$y-y_1=m(x-x_1)$.  Turns out that we are given both: }} \\
	&\\
	& Since the average velocity = 
	\ON<6->{\B{ slope of the secant line, then $m = 4$.}} \\
	&\\
	& Since the initial position is 5ft, then 
	\ON<7->{\B{$s(0)=5$ or $(x_1,y_1) = (0,5)$. }} \\
	&\\
	& Therefore, the equation of the secant line is: 
	\ON<8->{\B{$y-5=4(x-0)$ or $y=4x+5$}} \\
\end{tabular}}
...
%	\only<3>{\includegraphics[scale = 0.26]{Figures/01_01_Fig_13.png}}
%	\only<4>{\includegraphics[scale = 0.26]{Figures/01_01_Fig_19.png}}
%	\only<5>{\includegraphics[scale = 0.26]{Figures/01_01_Fig_20.png}}
%	\only<6>{\includegraphics[scale = 0.26]{Figures/01_01_Fig_22.png}}
%\end{center}
...
%$\only<2->{\OS{\B{ h \to 0  }}}{\only<2->{\OBT}{ \text{$h$ approaches $0$} }}$, 
%$\only<2->{\OS{\B{ S \to T  }}}{\only<2->{\OBT}{ \text{$S$ approaches $T$} }}$, and 
%$\only<2->{\OS{\B{ m_S \to m_T  }}}{\only<2->{\OBT}{ \text{$m_S$ approaches $m_T$} }}$ 
% 
%\ECOL\BCOL{0cm}\ECOL\ECOLS\vspace{8.2cm}\ECOL\BCOL{0cm}\ECOL\ECOLS
%\end{frame}
%
...
%	\only<2->{\OS{\B{ f(1) }}}{
%	\only<2->{\OBT}{ [1-1.8]^2 }} \, \big) = (1,0.64)$  \hspace{0.5cm} Pt 2:  $\big( 1+h, 
%	\only<2->{\OS{\B{ f(1+h) }}}{\only<2->{\OBT}{ [(1+h)-1.8]^2 }} \, \big) = (1+h,[h-0.8]^2)$ \\
%\end{tabular}}
%
%\tikzstyle{na} = [baseline=-2.5ex]
%\begin{tikzpicture}[overlay]
%	\ON<2->{\ARROWB (n2.\DS) \BENDL (n3.\DS);}
%\end{tikzpicture}
%
%\begin{tikzpicture}
%
